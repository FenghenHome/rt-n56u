...
wget --no-check-certificate https://cdn.jsdelivr.net/gh/VeleSila/yhosts/hosts -O hosts;sed -i "1 i\## update：$(date "+%Y-%m-%d %H:%M:%S")" hosts
if [ ! -f "hosts" ]; then
...
EOF
cd /etc/storage/dnsmasq/dns
wget --no-check-certificate https://cdn.jsdelivr.net/gh/vokins/yhosts/data/tvbox.txt -O tvhosts;sed -i "1 i\## update：$(date "+%Y-%m-%d %H:%M:%S")" tvhosts
if [ ! -f "tvhosts" ]; then
